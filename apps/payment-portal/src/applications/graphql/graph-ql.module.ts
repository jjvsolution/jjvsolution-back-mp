...
@Module({
          ...
          GQLModule.forRootAsync<ApolloDriverConfig>({
            ...
            useFactory: (config: ConfigService<ConfigurationsInterface>) => {
              ...
              return {
                autoSchemaFile: join(
                  process.cwd(),
                  'apps/payment-portal/src/applications/graphql/schema.gql',
                ),
                playground: false,
                subscriptions: {
                  'graphql-ws': true,
                },
                path: '/payment-portal/graphql',
                plugins,
                formatError: (
  ...
  providers: [
    ...
    /* LoggingPlugin, */ // se debe descomentar para ver logs, solo funciona al ejecutar solo la app de payment portal
    JwtService,
    TokenService,
    ...resolver,
  ],
})
export class GraphQlPaymentPortalModule {}
